...
{#if loaded}
	<div class="flex flex-col gap-1 my-1.5">
		...
		<div class=" flex w-full space-x-2">
			<div class="flex flex-1 bg-white/50 dark:bg-gray-800/50 backdrop-blur-md rounded-xl border border-gray-200 dark:border-gray-700">
				<div class=" self-center ml-1 mr-3">
					<Search className="size-3.5" />
				</div>
				<input
					class=" w-full text-sm pr-4 py-2 rounded-r-xl outline-hidden bg-transparent"
					bind:value={query}
			...
			<div>
				{#if $user?.role === 'admin'}
					...
					>
						<div
							class=" px-2 py-2 rounded-xl hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 dark:hover:text-white transition font-medium text-sm flex items-center space-x-1"
						>
	...
	<div class="mb-5 gap-3 grid lg:grid-cols-2 xl:grid-cols-3">
		{#each filteredItems as tool}
			<div
				class=" flex space-x-4 cursor-pointer w-full px-4 py-3 bg-white/50 dark:bg-gray-800/50 backdrop-blur-md hover:bg-white/70 dark:hover:bg-gray-800/70 rounded-xl transition-all duration-200 border border-gray-200 dark:border-gray-700 hover:shadow-lg"
			>
				<a
						...
						<div class=" flex-1 self-center">
							<Tooltip content={tool?.meta?.description ?? ''} placement="top-start">
								<div class=" font-semibold flex items-center gap-1.5">
									<div
										class=" text-xs font-bold px-1.5 py-0.5 rounded uppercase line-clamp-1 bg-gradient-to-r from-blue-500/20 to-purple-500/20 text-gray-700 dark:text-gray-200 border border-blue-500/30 dark:border-purple-500/30"
									>
										TOOL
									</div>
									...
									{#if tool?.meta?.manifest?.version}
										<div
											class="text-xs font-bold px-1.5 py-0.5 rounded line-clamp-1 bg-gradient-to-r from-green-500/20 to-teal-500/20 text-gray-700 dark:text-gray-200 border border-green-500/30 dark:border-teal-500/30"
										>
									...
									<div class="line-clamp-1">
										...
										<span class=" text-gray-600 dark:text-gray-400 text-xs font-medium shrink-0">{tool.id}</span>
									</div>
								</div>
							</Tooltip>
				...
				<div class="flex flex-row gap-0.5 self-center">
					{#if shiftKey}
						<Tooltip content={$i18n.t('Delete')}>
							<button
								class="self-center w-fit text-sm px-2 py-2 text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-200 dark:hover:bg-gray-700 rounded-xl"
								type="button"
								on:click={() => {
									deleteHandler(tool);
								}}
					...
					{:else}
						...
						>
							<button
								class="self-center w-fit text-sm p-1.5 text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-200 dark:hover:bg-gray-700 rounded-xl"
								type="button"
							>
	...
	{#if $user?.role === 'admin'}
		<div class=" flex justify-end w-full mb-2">
			<div class="flex space-x-2">
				...
				<button
					class="flex text-xs items-center space-x-1 px-4 py-2 rounded-xl bg-gradient-to-r from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800 hover:from-gray-200 hover:to-gray-300 dark:hover:from-gray-600 dark:hover:to-gray-700 text-gray-700 dark:text-gray-200 transition-all duration-200 shadow-sm hover:shadow-md"
					on:click={() => {
