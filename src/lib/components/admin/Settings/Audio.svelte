...
>
	<div class="space-y-4 overflow-y-scroll scrollbar-hidden h-full">
		<div class="flex flex-col gap-4">
			<div class="bg-gradient-to-r from-gray-800 to-gray-850 rounded-xl p-6 border border-gray-700/50">
				<div class="mb-4 text-lg font-semibold text-gray-100">{$i18n.t('STT-Einstellungen')}</div>

				<div class="mb-4 bg-gray-900/50 rounded-lg p-4">
					<div class="flex w-full justify-between items-center">
						<div class="text-sm font-medium text-gray-300">{$i18n.t('Sprache-zu-Text Engine')}</div>
						<div class="flex items-center relative">
							<select
								class="bg-gradient-to-r from-gray-700 to-gray-750 text-gray-200 cursor-pointer w-fit pr-8 rounded-lg px-3 py-2 text-sm border border-gray-600/50 outline-none focus:border-gray-500 transition-colors duration-200"
								bind:value={STT_ENGINE}
								placeholder="Engine auswählen"
							>
								<option value="">{$i18n.t('Whisper (Lokal)')}</option>
				...
				{#if STT_ENGINE === 'openai'}
					<div class="bg-gray-900/50 rounded-lg p-4 mb-4">
						<div class="flex gap-3">
							<input
								class="flex-1 w-full rounded-lg py-2.5 px-4 text-sm text-gray-200 bg-gradient-to-r from-gray-700 to-gray-750 border border-gray-600/50 outline-none focus:border-gray-500 transition-colors duration-200"
								placeholder={$i18n.t('API Basis-URL')}
								bind:value={STT_OPENAI_API_BASE_URL}
								required
							/>

							<SensitiveInput placeholder={$i18n.t('API-Schlüssel')} bind:value={STT_OPENAI_API_KEY} />
						</div>
					</div>
					...
					<div class="bg-gray-900/50 rounded-lg p-4">
						<div class="mb-2 text-sm font-medium text-gray-300">{$i18n.t('STT-Modell')}</div>
						<div class="flex w-full">
							<div class="flex-1">
								<input
									list="model-list"
									class="w-full rounded-lg py-2.5 px-4 text-sm text-gray-200 bg-gradient-to-r from-gray-700 to-gray-750 border border-gray-600/50 outline-none focus:border-gray-500 transition-colors duration-200"
				...
				{:else if STT_ENGINE === 'deepgram'}
					<div class="bg-gray-900/50 rounded-lg p-4 mb-4">
						<div class="flex gap-3">
							<SensitiveInput placeholder={$i18n.t('API-Schlüssel')} bind:value={STT_DEEPGRAM_API_KEY} />
						</div>
					</div>
						...
						<div class="flex w-full">
							<div class="flex-1">
								<input
									class="w-full rounded-lg py-2.5 px-4 text-sm text-gray-200 bg-gradient-to-r from-gray-700 to-gray-750 border border-gray-600/50 outline-none focus:border-gray-500 transition-colors duration-200"
									bind:value={STT_MODEL}
									placeholder="Modell auswählen (optional)"
								/>
							</div>
						</div>
						<div class="mt-2 text-xs text-gray-500">
							{$i18n.t('Modellfeld leer lassen, um das Standardmodell zu verwenden.')}
							<a
								class="hover:underline text-gray-400 hover:text-gray-300 transition-colors duration-200"
								href="https://developers.deepgram.com/docs/models"
								target="_blank"
							>
								{$i18n.t('Hier klicken, um verfügbare Modelle zu sehen.')}
							</a>
						</div>
					</div>
				{:else if STT_ENGINE === 'azure'}
					<div class="bg-gray-900/50 rounded-lg p-4 mb-4">
						<div class="flex gap-3">
							<SensitiveInput
								placeholder={$i18n.t('API-Schlüssel')}
					...
					<div class="bg-gray-900/50 rounded-lg p-4 mb-4">
						<div class="mb-2 text-sm font-medium text-gray-300">{$i18n.t('Azure-Region')}</div>
						<div class="flex w-full">
							<div class="flex-1">
								<input
									class="w-full rounded-lg py-2.5 px-4 text-sm text-gray-200 bg-gradient-to-r from-gray-700 to-gray-750 border border-gray-600/50 outline-none focus:border-gray-500 transition-colors duration-200"
									bind:value={STT_AZURE_REGION}
									placeholder={$i18n.t('z.B. westus (leer lassen für eastus)')}
								/>
							</div>
						</div>
					</div>
					...
					<div class="bg-gray-900/50 rounded-lg p-4 mb-4">
						<div class="mb-2 text-sm font-medium text-gray-300">{$i18n.t('Sprachregionen')}</div>
						<div class="flex w-full">
							<div class="flex-1">
								<input
									class="w-full rounded-lg py-2.5 px-4 text-sm text-gray-200 bg-gradient-to-r from-gray-700 to-gray-750 border border-gray-600/50 outline-none focus:border-gray-500 transition-colors duration-200"
									bind:value={STT_AZURE_LOCALES}
									placeholder={$i18n.t('z.B. en-US,ja-JP (leer lassen für Auto-Erkennung)')}
								/>
							</div>
						</div>
					</div>
					...
					<div class="bg-gray-900/50 rounded-lg p-4 mb-4">
						<div class="mb-2 text-sm font-medium text-gray-300">{$i18n.t('Endpunkt-URL')}</div>
						<div class="flex w-full">
							<div class="flex-1">
								<input
									class="w-full rounded-lg py-2.5 px-4 text-sm text-gray-200 bg-gradient-to-r from-gray-700 to-gray-750 border border-gray-600/50 outline-none focus:border-gray-500 transition-colors duration-200"
									bind:value={STT_AZURE_BASE_URL}
									placeholder={$i18n.t('(leer lassen für kommerziellen Endpunkt)')}
								/>
							</div>
						</div>
					</div>
					...
					<div class="bg-gray-900/50 rounded-lg p-4">
						<div class="mb-2 text-sm font-medium text-gray-300">{$i18n.t('Max. Sprecher')}</div>
						<div class="flex w-full">
							<div class="flex-1">
								<input
									class="w-full rounded-lg py-2.5 px-4 text-sm text-gray-200 bg-gradient-to-r from-gray-700 to-gray-750 border border-gray-600/50 outline-none focus:border-gray-500 transition-colors duration-200"
									bind:value={STT_AZURE_MAX_SPEAKERS}
									placeholder={$i18n.t('z.B. 3, 4, 5 (leer lassen für Standard)')}
								/>
							</div>
						</div>
					</div>
				{:else if STT_ENGINE === ''}
					<div class="bg-gray-900/50 rounded-lg p-4">
						...
						<div class="mt-2 mb-1 text-xs text-gray-400 dark:text-gray-500">
							{$i18n.t(`OAKMIND HIVE uses faster-whisper internally.`)}
			...
			<div>
				...
				{:else if TTS_ENGINE === 'transformers'}
					<div>
						...
						<div class="mt-2 mb-1 text-xs text-gray-400 dark:text-gray-500">
							{$i18n.t(`OAKMIND HIVE uses SpeechT5 and CMU Arctic speaker embeddings.`)}
